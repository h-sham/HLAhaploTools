...
collapse_genotypes <- function(df,
                               ...
                               quiet = FALSE,
                               isfamily = TRUE) {
  if (!quiet) message("\t🧬 Collapsing paired alleles into biallelic genotypes...")
  ...
  id_cols <- character()
  if (isfamily && all(c("FAMILY_ID", "Family_Member") %in% names(df))) {
    id_cols <- c("FAMILY_ID", "Family_Member")
    if (!quiet) message("\t✓ Using family data columns (FAMILY_ID, Family_Member)")
  }

  if (length(id_cols) == 0) {
    possible_id_cols <- c("SampleID", "Sample_ID", "ID", "Id", "id", "PATIENT_ID", "Patient_ID", "Subject_ID")
    found_ids <- intersect(possible_id_cols, names(df))
    id_cols <- if (length(found_ids) > 0) {
      found_ids[1]
    } else {
      if (!quiet) message("\t⚠️ No ID column found. Using row numbers.")
      df$ID <- seq_len(nrow(df))
      "ID"
    }
    if (!quiet && length(found_ids) > 0) message(sprintf("\t✓ Using ID column: %s", id_cols))
  }
  ...
  for (gene in loci) {
    ...
    if (!all(c(col1, col2) %in% names(df))) {
      if (!quiet) message(sprintf("\t⚠️ Skipping %s (missing %s or %s)", gene, col1, col2))
      next
    }
...
enumerate_diplotypes <- function(geno_row,
  ...
  id_cols <- c("ID", "FAMILY_ID", "Family_Member", "SampleID", "Sample_ID", "PATIENT_ID", "Patient_ID", "Subject_ID")
  skip_cols <- which(names(geno_row) %in% id_cols)
  geno_row_filtered <- if (length(skip_cols) > 0) geno_row[, -skip_cols, drop = FALSE] else geno_row

  loci <- names(geno_row_filtered)
  # Extract allele pairs into a list per locus using vapply
  allele_lists <- vapply(seq_along(loci), function(i) {
    gene <- loci[i]
  ...
  }, FUN.VALUE = list(list(gene = "", a1 = "", a2 = "")))

  # Remove NULL entries (for skipped loci)
  allele_lists <- allele_lists[vapply(allele_lists, Negate(is.null), logical(1))]
  ...
  if (!quiet && n_combos > 100) {
    message(sprintf("\t⚠️ Large number of diplotypes (%d) will be generated.", n_combos))
  }

  # Generate all phased combinations
...
compute_posteriors <- function(all_diplotypes,
  ...
  if (!is.numeric(hap_freqs) || any(hap_freqs < 0) || any(hap_freqs > 1)) {
  ...
  if (!quiet) {
    non_null <- sum(vapply(result, Negate(is.null), logical(1)))
...
run_em_algorithm <- function(all_diplotypes,
  ...
  n_samples <- sum(vapply(all_diplotypes, length, integer(1)) > 0)
  if (n_samples == 0) stop("\t❌ No valid diplotypes found.")
  ...
  if (length(all_haps) == 0) stop("\t❌ No haplotypes found in diplotype lists.")
  ...
  while (!converged && iter < max_iter) {
    ...
    for (i in seq_along(diplotype_hap_pairs)) {
      ...
      subject_posteriors <- vapply(subject_pairs, function(pair) {
        h1 <- pair[1]
        h2 <- pair[2]
        if (!h1 %in% names(hap_freqs) || !h2 %in% names(hap_freqs)) {
          return(0)
      ...
      for (j in seq_len(length(subject_pairs))) {
  ...
  if (!quiet) {
    if (converged) {
      message(sprintf("\t✅ EM algorithm converged after %d iterations.", iter))
    } else {
...
infer_haplotypes <- function(df,
                             ...
                             quiet = FALSE,
                             isfamily = FALSE) {
  if (!quiet) message("\t🔄 Starting HLA haplotype inference pipeline...")
  ...
  if (is.null(loci)) {
    ...
    message(sprintf("\t✅ Using user-specified loci: %s", paste(loci, collapse = ", ")))
  }

  if (length(loci) == 0) stop("\t❌ No HLA loci found for analysis.")
  ...
  if (!quiet) message("\t🔢 Enumerating all possible diplotypes per subject...")

  all_diplotypes <- lapply(seq_len(nrow(genotypes)), function(i) {
    enumerate_diplotypes(genotypes[i, , drop = FALSE], quiet = TRUE)
  })

  valid_samples <- sum(vapply(all_diplotypes, length, integer(1)) > 0)

  if (!quiet) {
    message(sprintf(
      "\t✅ Found valid diplotype configurations for %d out of %d samples.",
      valid_samples, nrow(genotypes)
    ))
  }

  if (valid_samples == 0) stop("\t❌ No valid diplotypes found to analyze.")
  ...
  # Step 7: Identify most likely diplotype per subject
  if (!quiet) message("\t🏆 Identifying most likely diplotype for each subject...")

  # Determine ID columns
  if (isfamily && all(c("FAMILY_ID", "Family_Member") %in% names(genotypes))) {
    id_cols <- c("FAMILY_ID", "Family_Member")
  } else {
    possible_id_cols <- c("ID", "SampleID", "Sample_ID", "PATIENT_ID", "Subject_ID")
    id_cols <- intersect(possible_id_cols, names(genotypes))
    if (length(id_cols) == 0) id_cols <- "ID"
  }
  ...
  for (i in seq_along(all_diplotypes)) {
    ...
    if (length(subject_diplotypes) == 0 || is.null(subject_posteriors) || all(subject_posteriors == 0)) {
      next
    }
    ...
    top_diplotypes$probability[i] <- subject_posteriors[best_idx]
  }

  # Step 8: Format haplotype frequency results
  if (!quiet) message("\t📋 Preparing haplotype frequency results...")
...
#' p <- plot_top_haplotypes(result$haplotype_frequencies, n_top = 10, min_freq = 0.02)
#' }
...
plot_top_haplotypes <- function(hap_freq,
  ...
  if (nrow(plot_data) > n_top) {
    original_count <- nrow(plot_data)
    plot_data <- plot_data[seq_len(n_top), ]
    if (!quiet) {
      message(sprintf("\t✓ Showing top %d haplotypes out of %d that meet threshold.", n_top, original_count))
    }
  }

  plot_data$haplotype <- factor(
    plot_data$haplotype,
    levels = rev(plot_data$haplotype)
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::geom_text(
      ggplot2::aes(label = sprintf("%.1f%%", frequency * 100)),
      hjust = -0.2
    ) +
    ...
    ggplot2::coord_flip() +
    ggplot2::labs(
      title = "Top HLA Haplotype Frequencies",
      x = "Haplotype",
      y = "Frequency"
    ) +
