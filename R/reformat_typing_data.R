...
#' Rows are reordered by FAMILY_ID and Family_Member with levels F, M, C1, C2.
#'
...
#'   filled missing second alleles, and rows ordered by family.
...
reformat_typing_data <- function(df) {
  ...
  # Fill missing _2 alleles with _1 if _1 is present
  for (gene in unique(sub("_.*", "", all_allele_cols))) {
    ...
    if (allele1 %in% names(df) && allele2 %in% names(df)) {
      df[[allele2]] <- ifelse((is.na(df[[allele2]]) |
                                 df[[allele2]] == "") & !is.na(df[[allele1]]) & df[[allele1]] != "", df[[allele1]], df[[allele2]])
    }
  }
  ...
  # Reorder rows by FAMILY_ID and Family_Member factor levels
  desired_order <- c("F", "M", "C1", "C2")
  if ("Family_Member" %in% names(df)) {
    df <- dplyr::mutate(df,
