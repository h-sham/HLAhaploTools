...
#'
#' @export
compute_hla_segregation2 <- function(hped, collapse = "~") {
      ...
      if (!is_parent) {
         ...
         for (gene in gene_names) {
            ...
            f1 <- if (nrow(dad) > 0 &&
               paste0(gene, "_1") %in%
                  names(dad)) {
               as.character(dad[[paste0(gene, "_1")]][1])
            } else {
               NA
            }
            f2 <- if (nrow(dad) > 0 &&
               paste0(gene, "_2") %in%
                  names(dad)) {
               as.character(dad[[paste0(gene, "_2")]][1])
            } else {
               NA
            }
            m1 <- if (nrow(mom) > 0 &&
               paste0(gene, "_1") %in%
                  names(mom)) {
               as.character(mom[[paste0(gene, "_1")]][1])
            } else {
               NA
            }
            m2 <- if (nrow(mom) > 0 &&
               paste0(gene, "_2") %in%
                  names(mom)) {
               as.character(mom[[paste0(gene, "_2")]][1])
            } else {
               NA
            }
